...
CONFIG(debug, debug|release) {
    TARGET = libqtoufi-d
    TARGET_PATH = $$shell_path($$OUT_PWD/debug)
}
else {
    TARGET = libqtoufi
    TARGET_PATH = $$shell_path($$OUT_PWD/release)
}
...
windows {
    QMAKE_POST_LINK += bash $$shell_quote($$PWD/post_build_win.sh) $$shell_quote($$shell_path($$PWD/../bin/win)) $$shell_quote($$TARGET_PATH) ;
}
unix {
    QMAKE_POST_LINK += bash $$shell_quote($$PWD/post_build_unix.sh) $$shell_quote($$shell_path($$PWD/../bin/unix)) $$shell_quote($$TARGET_PATH) ;
